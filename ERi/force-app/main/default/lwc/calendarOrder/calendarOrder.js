...
import getOrd from '@salesforce/apex/OrderFullCalendarService.getOrd';
//import heySfLogo from '@salesforce/resourceUrl/heySfLogo';
/**
 * @description Full Calendar JS - Lightning Web Components
 */
export default class calendarOrder extends LightningElement {
//  @track eventData ;
  @track returnedOrd = [] ;
  @track finalOrd = [] ;
  // SfLogo = heySfLogo ;
  ...
  initialiseFullCalendarJs() {
    ...
    for(var i = 0 ; i < this.returnedOrd.length ; i++)
    {
      this.finalOrd.push({
        start : this.returnedOrd[i].EffectiveDate,
        title : this.returnedOrd[i].OrderNumber,
        url : tDomain+'/lightning/r/Order/'+this.returnedOrd[i].Id+'/view'
    });
    }
    ...
    $(ele).fullCalendar({
     ...
     // defaultDate: '2020-03-12',
      ...
      events : this.finalOrd
    });
  }
  getTasks(){
    getOrd()
        .then(result =>{   
           this.returnedOrd = JSON.parse(result);
